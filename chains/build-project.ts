...
export default defineChain({
  ...
  setup({ params, step }) {
    ...
    const { answer } = step("prompt_completion", {
      system_prompt: `You are a cloud architect working in AWS. You have been tasked with designing a solution to a problem. The problem is: ${problemStatement}. The solution must meet the following requirements: ${expectations}. All Terraform variables except for the AWS credentials should come with sensible defaults. If a non-HCL file is required for the solution, it should be added to the JSON.`,
